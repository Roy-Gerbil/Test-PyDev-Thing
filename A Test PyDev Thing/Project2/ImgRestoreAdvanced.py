    ...
    for x in range(0, I.shape[0]):
        for y in range(0, I.shape[1]):
            ...
            if (mask[x,y] == 0 and mask[x,y+1] == 1): # Bottom
                Boundary[x,y] = 4
            if (mask[x,y] == 0 and mask[x-1,y] == 1 and mask[x,y-1] == 1): #Left - Top
                Boundary[x,y] = 5
            if (mask[x,y] == 0 and mask[x-1,y] == 1 and mask[x,y+1] == 1): #Left - Bottom
                Boundary[x,y] = 6
            if (mask[x,y] == 0 and mask[x+1,y] == 1 and mask[x,y-1] == 1): #Right - Top
                Boundary[x,y] = 7
            if (mask[x,y] == 0 and mask[x+1,y] == 1 and mask[x,y+1] == 1): #Right - Bottom
                Boundary[x,y] = 8
    return(Boundary)
